...
export function SaasManagementPage() {
  ...
  const [tab, setTab] = useState<'overview' | 'branding' | 'public-intake' | 'team' | 'security' | 'organizations'>('overview');
  ...
  return <div className="page"><header className="page-head"><div><span className="eyebrow">Administración SaaS</span><h1>Espacio de trabajo</h1><p>Gestiona organización, identidad visual, radicación pública, equipo, consumo y aislamiento multiempresa.</p></div><div className="page-actions"><span className="chip tone-purple">{context.subscription.plan.name}</span><button className="btn btn-soft" onClick={reload}><RefreshCw size={17} /> Actualizar</button></div></header>{warning ? <div className="alert danger">{warning}</div> : null}<div className="phase56-admin-tabs phase8-tabs"><button className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}><Gauge /> Resumen</button>{context.canManage ? <button className={tab==='branding'?'active':''} onClick={()=>setTab('branding')}><Palette /> Marca</button> : null}{context.canManage ? <button className={tab==='public-intake'?'active':''} onClick={()=>setTab('public-intake')}><Globe2 /> Radicación pública</button> : null}{context.canManage ? <button className={tab==='team'?'active':''} onClick={()=>setTab('team')}><Users /> Equipo</button> : null}{context.canManage ? <button className={tab==='security'?'active':''} onClick={()=>setTab('security')}><ShieldCheck /> Seguridad</button> : null}<button className={tab==='organizations'?'active':''} onClick={()=>setTab('organizations')}><Building2 /> Organizaciones</button></div>{tab==='overview'?<WorkspaceOverview context={context}/>:null}{tab==='branding'&&context.canManage?<BrandingForm context={context} onSaved={reload}/>:null}{tab==='public-intake'&&context.canManage?<PublicIntakeSettingsPanel context={context} onSaved={reload}/>:null}{tab==='team'&&context.canManage?<TeamInvitations context={context} onChanged={reload}/>:null}{tab==='security'&&context.canManage?<SecurityHealthPanel/>:null}{tab==='organizations'?<OrganizationsPanel context={context}/>:null}</div>;
}
...
export function InvitationPage() {
  ...
  async function accept() { if (!token) return; setAccepting(true); setMessage(''); try { await sigcService.acceptOrganizationInvitation(token); navigate('/'); } catch (acceptError) { setMessage(acceptError instanceof Error ? acceptError.message : 'No fue posible aceptar.'); } finally { setAccepting(false); } }
  async function signUp(event: FormEvent) { event.preventDefault(); if (!supabase || !invitation) return; setAccepting(true); setMessage(''); try { const { data, error: signUpError } = await supabase.auth.signUp({ email: invitation.email, password, options: { data: { name } } }); if (signUpError) throw signUpError; if (data.session) { await sigcService.acceptOrganizationInvitation(token!); navigate('/'); } else setMessage('Cuenta creada. Confirma tu correo y vuelve a abrir este enlace para aceptar la invitación.'); } catch (signUpError) { setMessage(signUpError instanceof Error ? signUpError.message : 'No fue posible crear la cuenta.'); } finally { setAccepting(false); } }
