...
function OrganizationsPanel({ context }: { context: SigcSaasContext }) {
  ...
  async function create(event: FormEvent) { event.preventDefault(); setSaving(true); setMessage(''); try { await sigcService.createSaasOrganization({ name, slug }); window.location.assign('/app'); } catch (error) { setMessage(error instanceof Error ? error.message : 'No fue posible crear la organización.'); setSaving(false); } }
...
export function InvitationPage() {
  ...
  async function accept() { if (!token) return; setAccepting(true); setMessage(''); try { await sigcService.acceptOrganizationInvitation(token); navigate('/app'); } catch (acceptError) { setMessage(acceptError instanceof Error ? acceptError.message : 'No fue posible aceptar.'); } finally { setAccepting(false); } }
