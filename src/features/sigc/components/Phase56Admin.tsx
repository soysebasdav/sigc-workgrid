  ...
  return (
    <div className="page phase56-admin-page">
      <header className="page-head">
        <div>
          <span className="eyebrow">Fases 5 y 6 · Configuración sobre programación</span>
          <h1>Administración y automatizaciones</h1>
          <p>Parametriza el SIGC y construye reglas CUANDO → SI → ENTONCES sin modificar el código.</p>
        </div>
...
function SlaPanel({ data, showToast }: { data: SigcAdminSnapshot; showToast: (text: string) => void }) {
  ...
  return <div className="phase56-stack-sections">
    ...
    {reminderEdit ? <ReminderModal item={reminderEdit === 'new' ? null : reminderEdit} onClose={() => setReminderEdit(null)} showToast={showToast} /> : null}
  </div>;
}
...
function ReminderModal({ item, onClose, showToast }: { item: AdminReminderRule | null; onClose: () => void; showToast: (text: string) => void }) {
  const [form, setForm] = useState<SaveReminderRuleInput>({ id: item?.id, code: item?.code ?? '', name: item?.name ?? '', triggerKind: item?.triggerKind ?? 'before_due', offsetMinutes: item?.offsetMinutes ?? 1440, includeManagers: item?.includeManagers ?? false, isActive: item?.isActive ?? true });
...
function WorkflowsPanel({ data, showToast }: { data: SigcAdminSnapshot; showToast: (text: string) => void }) {
  ...
  async function saveStates() { try { await sigcService.saveWorkflowStates(caseTypeId, stateIds); showToast('Estados del flujo actualizados.'); } catch (error) { showToast(errorMessage(error)); } }
...
function TransitionModal({ data, caseTypeId, item, onClose, showToast }: { data: SigcAdminSnapshot; caseTypeId: string; item: AdminTransition | null; onClose: () => void; showToast: (text: string) => void }) {
  const [form, setForm] = useState<SaveTransitionInput>({ id: item?.id, caseTypeId, fromStateId: item?.fromStateId ?? data.states[0]?.id ?? '', toStateId: item?.toStateId ?? data.states[1]?.id ?? '', requiredPermissionCode: item?.requiredPermissionCode ?? '', requiresJustification: item?.requiresJustification ?? false, isActive: item?.isActive ?? true });
  async function submit(event: FormEvent) { event.preventDefault(); try { await sigcService.saveTransition(form); showToast('Transición guardada.'); onClose(); } catch (error) { showToast(errorMessage(error)); } }
  return <ConfigModal title={item ? 'Editar transición' : 'Nueva transición'} onClose={onClose}><form className="stack" onSubmit={submit}><div className="form-grid two"><label className="field-label">Desde<select className="input" value={form.fromStateId} onChange={(e) => setForm({ ...form, fromStateId: e.target.value })}>{data.states.map((state) => <option key={state.id} value={state.id}>{state.name}</option>)}</select></label><label className="field-label">Hacia<select className="input" value={form.toStateId} onChange={(e) => setForm({ ...form, toStateId: e.target.value })}>{data.states.map((state) => <option key={state.id} value={state.id}>{state.name}</option>)}</select></label></div><label className="field-label">Permiso requerido<select className="input" value={form.requiredPermissionCode ?? ''} onChange={(e) => setForm({ ...form, requiredPermissionCode: e.target.value || undefined })}><option value="">Sin permiso específico</option>{data.permissions.map((permission) => <option key={permission.id} value={permission.code}>{permission.name} · {permission.code}</option>)}</select></label><div className="phase56-check-grid"><CheckField label="Exigir justificación" checked={form.requiresJustification} onChange={(checked) => setForm({ ...form, requiresJustification: checked })} /><CheckField label="Activa" checked={form.isActive} onChange={(checked) => setForm({ ...form, isActive: checked })} /></div><button className="btn btn-primary full"><Save size={16} /> Guardar transición</button></form></ConfigModal>;
}
...
function TemplatesPanel({ data, showToast }: { data: SigcAdminSnapshot; showToast: (text: string) => void }) {
  ...
  return <div><SectionHead title="Plantillas de correo" description="Personaliza asunto y contenido con variables como {{radicado}}, {{asunto}} y {{fecha_limite}}." action={<button className="btn btn-primary" onClick={() => setEditing('new')}><Plus size={16} /> Nueva plantilla</button>} /><div className="phase56-template-grid">{data.emailTemplates.map((item) => <article key={item.id}><header><div className="kpi-icon"><Mail size={18} /></div><StatusPill active={item.isActive} /></header><strong>{item.name}</strong><code>{item.code}</code><span>{item.subject}</span><p>{item.bodyText.slice(0, 180)}{item.bodyText.length > 180 ? '…' : ''}</p><button className="btn btn-white full" onClick={() => setEditing(item)}><Edit3 size={15} /> Editar plantilla</button></article>)}</div>{editing ? <TemplateModal item={editing === 'new' ? null : editing} onClose={() => setEditing(null)} showToast={showToast} /> : null}</div>;
}

function TemplateModal({ item, onClose, showToast }: { item: AdminEmailTemplate | null; onClose: () => void; showToast: (text: string) => void }) {
  const [form, setForm] = useState<SaveEmailTemplateInput>({ id: item?.id, code: item?.code ?? '', name: item?.name ?? '', eventType: item?.eventType ?? '', subject: item?.subject ?? '', bodyText: item?.bodyText ?? '', isActive: item?.isActive ?? true });
  ...
  return <ConfigModal title={item ? 'Editar plantilla' : 'Nueva plantilla'} onClose={onClose}><form className="stack" onSubmit={submit}><div className="form-grid two"><label className="field-label">Código<input className="input" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} required /></label><label className="field-label">Nombre<input className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label></div><label className="field-label">Evento<input className="input" value={form.eventType} onChange={(e) => setForm({ ...form, eventType: e.target.value })} placeholder="case_created" /></label><label className="field-label">Asunto<input className="input" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} required /></label><label className="field-label">Contenido<textarea className="input textarea phase56-template-body" value={form.bodyText} onChange={(e) => setForm({ ...form, bodyText: e.target.value })} required /></label><CheckField label="Activa" checked={form.isActive} onChange={(checked) => setForm({ ...form, isActive: checked })} /><button className="btn btn-primary full"><Save size={16} /> Guardar plantilla</button></form></ConfigModal>;
}
...
function AutomationModal({ data, item, onClose, showToast }: { data: SigcAdminSnapshot; item: AutomationRule | null; onClose: () => void; showToast: (text: string) => void }) {
  const [form, setForm] = useState<SaveAutomationRuleInput>({ id: item?.id, code: item?.code ?? '', name: item?.name ?? '', description: item?.description ?? '', triggerEvent: item?.triggerEvent ?? 'case.created', conditions: item?.conditions ?? [], actions: item?.actions ?? [], stopOnError: item?.stopOnError ?? true, sortOrder: item?.sortOrder ?? 0, isActive: item?.isActive ?? true });
...
function ActionFields({ data, action, onChange }: { data: SigcAdminSnapshot; action: AutomationAction; onChange: (value: AutomationAction) => void }) {
  ...
  return <div className="phase56-action-note"><CheckCircle2 size={17} /> Notifica a administración cuando todas las subtareas activas están completadas.</div>;
}
