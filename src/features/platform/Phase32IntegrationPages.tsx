...
function PageHead({ title, description, actions, platform = true }: { title: string; description: string; actions?: ReactNode; platform?: boolean }) {
  return <header className={platform ? 'platform-page-head' : 'page-head'}><div><span className="eyebrow">Orkesta · Fase 3.2 integraciones</span><h1>{title}</h1><p>{description}</p></div>{actions ? <div className={platform ? 'platform-page-actions' : 'page-actions'}>{actions}</div> : null}</header>;
}
...
function Modal({ title, subtitle, close, children }: { title: string; subtitle: string; close: () => void; children: ReactNode }) {
  return <div className="platform-modal-backdrop"><section className="card platform-modal commercial-modal-wide"><header><div><span className="eyebrow">Fase 3.2</span><h2>{title}</h2><p>{subtitle}</p></div><button type="button" onClick={close}><X /></button></header>{children}</section></div>;
}
