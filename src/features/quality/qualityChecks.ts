...
function sampleReport(): { report: SigcReportResult; rows: SigcReportRow[] } {
  const row: SigcReportRow = {
    ...
    requesterName: 'Prueba de calidad',
    requesterCompany: 'SIGC',
    source: 'manual',
    riskLevel: 'Bajo',
...
export async function runClientQualityChecks(): Promise<QualityCheckResult[]> {
  ...
  checks.push(runCheck('EXPORT-PDF-SIGNATURE', 'integration', 'PDF generado tiene cabecera válida', async () => {
    const { report, rows } = sampleReport();
    const text = await createPdfBlob(report, rows).text();
    const passed = text.startsWith('%PDF-1.4') && text.includes('%%EOF');
    return { passed, details: passed ? 'PDF 1.4 con cierre EOF válido.' : 'La estructura PDF básica no es válida.', evidence: { size: text.length } };
  }));
